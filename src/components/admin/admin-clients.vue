...
<!--        <br>-->
        ...
        <table class="admin-table table table-striped table-condensed">
          ...
          <tbody>
            <tr v-for="(item, index) in clientList" v-bind:key='item.id' @click="openEditMenu('/admin/clients/edit/' + item.id)">
              <td>{{(page*pageSize) + (index + 1)}}</td>
              <td>{{item.name}}</td>
              <td class='table-bod'>{{getBirthDateString(item.birthday)}}<span v-html='getAge(item.birthday)'></span></td>
              <td>{{item.phone}}</td>
    ...
    <div class="admin-modal-wrapper" v-if='modal1'>
      <div class="admin-modal">
        <h4 class="admin-header">Список телефонов</h4>  
        <button class="admin-modal-close" @click='closeModal()'>Закрыть</button>
        <table class="admin-table table table-striped table-condensed">
          <thead>
            <th>
                #
          ...
          <tbody>
            <tr v-for="(item, index) in phoneList" v-bind:key='index'>
              <td>{{(page*pageSize) + (index + 1)}}</td> 
              <td>{{item.name}}</td>
              <td>{{item.phone}}</td>
              <td>
                <span v-for="(order, i) in item.orders" v-bind:key='i'>
                  {{ order + semicolon(i, item.orders.length) }} 
                </span>
              </td>
            </tr>
          </tbody>
    ...
    <div class="admin-modal-wrapper" v-if='modal2'>
      <div class="admin-modal">
        <h4 class="admin-header">Список почт</h4>  
        <button class="admin-modal-close" @click='closeModal()'>Закрыть</button>
        <table class="admin-table table table-striped table-condensed">
          <thead>
            <th>
          ...
          <tbody>
            <tr v-for="(item, index) in mailList" v-bind:key='index'>
              <td>{{index + 1}}</td>
              <td>{{item.name}}</td>
              <td v-if='item.mails.length > 0'>
                <span v-for="(order, i) in item.mails" v-bind:key='i'>
                  {{ order + semicolon(i, item.mails.length) }} 
                </span>
              </td>
              <td v-else>
