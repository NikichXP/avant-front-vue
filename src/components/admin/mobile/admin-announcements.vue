...
export default {
  name: 'adminMobileAnnouncementsComponent',
  data () {
    return {
      clientList: [],
      page: 0,
    ...
    generatePosts: function (count) {
      for (let i = 0; i < count; i++)  
        this.posts.push({
          ...
          userName: Math.round(Math.random() * 100000),
          advName: Math.round(Math.random() * 100000),
          rating: Math.round(Math.random() * 10),
          userRating: Math.round(Math.random() * 10),
          text: Math.round(Math.random() * 100000)
        })  
